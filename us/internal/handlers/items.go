...
func (ch *CatalogHandler) AddItemHandler(c *fiber.Ctx) error {
	...
	/*existingItem, _ := ch.App.S.GetItemByName(newItem.Name)
	if existingItem != nil {
...
func (ch *CatalogHandler) GetItemByNameHandler(c *fiber.Ctx) error {
	itemName := c.Params("ItemName")

	item, err := ch.App.S.GetItemByName(itemName)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "Failed to get item"})
	}
